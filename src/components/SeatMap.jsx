...
export default function SeatMap({ flight, isOpen, onClose, onConfirm }) {
  ...
  const getSeatClassAndPrice = (row) => {
    if (row <= 2) return { type: '頭等艙', price: 15000, color: '#f59e0b', bg: 'rgba(245,158,11,0.2)' }; // First
    if (row <= 5) return { type: '豪華經濟艙', price: 5000, color: '#8b5cf6', bg: 'rgba(139,92,246,0.2)' }; // Premium
    if (row === 6) return { type: '逃生口大空間', price: 2000, color: '#10b981', bg: 'rgba(16,185,129,0.2)' }; // Exit
    return { type: '標準經濟艙', price: 500, color: '#3b82f6', bg: 'rgba(59,130,246,0.2)' }; // Economy
  };
